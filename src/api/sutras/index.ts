...
export class SutrasApi {
  ...
  static async getSutras(request: FormData): Promise<SutraDetail[]> {
    const response = await apiGet("/sutras", request);
    return response;
  }

  static async putSutras(
    request: Partial<Sutra & Pick<Sutra, "id">>
  ) {
