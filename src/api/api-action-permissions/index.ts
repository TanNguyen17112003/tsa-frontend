...
export class ApiActionPermissionsApi {
  ...
  static async getApiActionPermissions(
    request: FormData
  ): Promise<{ api_action_id: number; permission_id: number; api_action_name: string }[]> {
  ...
  static async updateApiActionPermissionsByApiAction(request: {
    ...
    const response = await apiPut('/api_action_permissions/all/' + request.permission_id, request);
    return response;
  }
